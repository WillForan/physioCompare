...
for(i in 1:nrow(node.connections.orig)) {roi=node.connections.orig[i,'roinum']
 ...
 name= gsub(' ?','',as.character(roi.lables$name[idx]))
...
toptenhist <- ggplot(subset(ageeff.highlyconn,ROI %in% names(hubs)[1:13] ),aes(x=ageXphysio.val,fill=ROIname))+geom_histogram(stat="bin")+theme_bw()+scale_fill_brewer(type='qual',palette=3)
